parRF <- function(parVar = ".", 
  ...
  return(function(Rsp, predT, res,
    ...
    if (dim(Train.data)[2] == 2){
      datRf_temp <- Train.data[, -which(names(Train.data) == Rsp)]
      datRf <- data.frame(temp = datRf_temp) 
      names(datRf) <- names(Train.data)[-which(names(Train.data) == Rsp)]
    }else{
    ...
    if (is.null(mtry)){
      mtry <- max(floor( length(parVarNew)/3),1)
    }
