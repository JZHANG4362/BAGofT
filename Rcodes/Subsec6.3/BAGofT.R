...
BAGofT <- function(testModel, parFun, 
                   data, nsplits = 100, ne = floor(5*nrow(data)^(1/2)), nsim = 100){
  if (nsplits < 1)stop("nsplits should be at least 1")
  ...
  if (nsim >= 1){
    ...
    modFit <- testModel(Train.data = data, Test.data = dataFittemp)
    ...
    for (i in c(1:nsim)){
      ...
      ydat2 <- sapply(pdat2, function(x) rbinom(1, 1, x))
      dat2 <- data
      Rsp <- modFit$Rsp
      dat2[,Rsp] <- ydat2
